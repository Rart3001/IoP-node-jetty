 ...
 * @since Java JDK 1.7
 */
public class MessageTransmitSyncProcessor extends PackageProcessor {

    /**
     * Represent the LOG
     */
    private final Logger LOG = Logger.getLogger(ClassUtils.getShortClassName(MessageTransmitSyncProcessor.class));
     ...
     * Constructor
     */
    public MessageTransmitSyncProcessor() {
                ...
                clientDestination.getAsyncRemote().sendObject(packageReceived, new SendHandler() {
                    ...
                    try {
                        if (result.isOK()) {
                            ...
                            channel.sendPackage(session, messageTransmitRespond.toJson(), packageReceived.getNetworkServiceTypeSource(), PackageType.MESSAGE_TRANSMIT_SYNC_ACK_RESPONSE, destinationIdentityPublicKey);
